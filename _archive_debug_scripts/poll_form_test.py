        ...
        if results:
            ...
            if eid > LATEST:
                ...
                if st in ('success', 'error', 'crashed'):
                    ...
                    # Confirm snapshot has new code
                    snap_nodes = full.get('workflowData', {}).get('nodes', [])
                    for n in snap_nodes:
                        if n.get('name') == 'Parse Offer Data':
                            code = n['parameters'].get('jsCode', '')
                            print(f'Snapshot: Parse Offer Data code len={len(code)}, has dbg_p0={"dbg_p0" in code}')
                            break
                    ...
                    # Count nodes that ran
                    print(f'Nodes ran: {len(run_data) if isinstance(run_data, dict) else "?"}')

                    # Check trigger type
                    for trigger in ['📝 Form — New Provider', '🔗 Webhook — New Provider API']:
                        if trigger in (run_data or {}):
                            print(f'Triggered via: {trigger}')
                            break
                                    ...
                                    if 'Template' in str(fname):
                                        try:
                                            ...
                                            if isinstance(b64, str):
                                                ...
                                                ws = wb['Offers']
                                                print('  Offers sheet sample rows:')
                                                sizes = set(); tiers = set()
                                                for row in ws.iter_rows(min_row=3, max_row=22, values_only=True):
                                                    r_vals = list(row)
                                                    if r_vals[1]: sizes.add(r_vals[1])
                                                    if r_vals[2]: tiers.add(r_vals[2])
                                                for row in ws.iter_rows(min_row=3, max_row=7, values_only=True):
                                                    vals = [str(c)[:28] if c else '' for c in list(row)[1:6]]
                                                    print(f'    {vals}')
                                                print(f'  Party sizes present: {sorted(sizes)}')
